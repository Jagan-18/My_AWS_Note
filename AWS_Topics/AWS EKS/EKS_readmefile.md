...
## 🧩 **What is AWS EKS?**
AWS EKS is a **managed Kubernetes service** that lets you run Kubernetes workloads without having to install, operate, and maintain your own Kubernetes control plane.

* AWS manages the **control plane** (API server, etcd, scheduler, controller manager).
* You manage the **worker nodes** (EC2 or Fargate) and the Kubernetes resources (pods, deployments, services, etc.).

---
## 🔹 **Core Components of EKS**
Kubernetes itself has a standard architecture — EKS just runs it on AWS.

### 1️⃣ **Control Plane** *(Managed by AWS)*
* **Kubernetes API Server** – Entry point for all Kubernetes commands.
* **etcd** – Key-value store for cluster state.
* **Scheduler** – Assigns pods to nodes.
* **Controller Manager** – Manages state reconciliation (deployments, replicas, etc.).
* AWS automatically scales & patches it, ensuring HA (multi-AZ).

---
### 2️⃣ **Worker Nodes** *(Managed by you)*
* Can be:

  * **EC2 Instances** – Self-managed, more control.
  * **Fargate** – Serverless compute for pods.
* Runs:

  * **Kubelet** – Talks to the API server.
  * **Container Runtime** – Usually containerd or Docker.
  * **kube-proxy** – Handles networking to other pods/services.

---
### 3️⃣ **Kubernetes Objects**

You still define & manage your Kubernetes workloads:

* **Pods** – Smallest deployable unit, contains 1+ containers.
* **Deployments** – Declarative updates for pods.
* **Services** – Networking endpoints (ClusterIP, NodePort, LoadBalancer).
* **Ingress** – Layer 7 routing.
* **ConfigMaps & Secrets** – Store configuration & sensitive data.

---
### 4️⃣ **Networking in EKS**

* Runs inside a **VPC**.
* Uses **AWS VPC CNI plugin** so pods get their own VPC IP addresses.
* Supports **Security Groups** and **Network Policies**.

---
### 5️⃣ **EKS Integrations with AWS**

* **IAM** – Fine-grained role-based access to Kubernetes.
* **CloudWatch** – Metrics & logging.
* **ECR** – Pull private images.
* **ALB/NLB** – Expose workloads to the internet.
...
---
## 🔄 **EKS Workflow Example**

1. You push an image to **ECR**.
2. You apply Kubernetes manifests (`kubectl apply -f deployment.yaml`).
3. The **API Server** schedules pods to available nodes.
4. Pods run containers using the image from ECR.
5. A **Service + Load Balancer** exposes your app to users.
...
| Portability    | Runs anywhere with Kubernetes | AWS only             |
| Learning curve | Higher (K8s concepts)         | Easier               |
| Flexibility    | More customization            | Simpler, opinionated |

---
## 🧠 **Interview Tip**
If asked “What is AWS EKS?” say:

> *"AWS EKS is a managed Kubernetes service where AWS runs the Kubernetes control plane for you, while you manage worker nodes and workloads. It integrates with AWS networking, IAM, monitoring, and storage services, allowing you to run scalable, containerized applications without managing the Kubernetes masters yourself."*
